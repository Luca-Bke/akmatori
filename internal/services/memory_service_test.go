...
func TestMemoryService_SetSuppress_FlipsFlag(t *testing.T) {
	svc := setupMemoryServiceTest(t)

	created, err := svc.CreateMemory(validMemory("suppress-target"))
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if created.Suppress {
		t.Fatal("expected suppress=false after create")
	}

	if err := svc.SetSuppress(created.ID, true); err != nil {
		t.Fatalf("SetSuppress(true): %v", err)
	}

	fetched, err := svc.GetMemory(created.ID)
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	if !fetched.Suppress {
		t.Errorf("expected suppress=true after SetSuppress(true)")
	}

	// Flip back.
	if err := svc.SetSuppress(created.ID, false); err != nil {
		t.Fatalf("SetSuppress(false): %v", err)
	}
	fetched, _ = svc.GetMemory(created.ID)
	if fetched.Suppress {
		t.Errorf("expected suppress=false after SetSuppress(false)")
	}
}

func TestMemoryService_SetSuppress_NotFound(t *testing.T) {
	svc := setupMemoryServiceTest(t)
	err := svc.SetSuppress(999, true)
	if !IsMemoryNotFoundErr(err) {
		t.Errorf("expected not-found error for unknown ID, got %v", err)
	}
}
